...
def conv2d(X, W, bias):
    ...
    X_out_re = X_out.reshape((batch_size, out_channels, nl.par_dim(1), out_pool_height*out_pool_width)) 
    ...
    for b in nl.sequential_range(batch_size):
        ...
        for o in nl.sequential_range(out_channels): # just one pixel depth at a time
            ...
            for p in nl.sequential_range(num_pixels_per_in_channel // tile_size_pixels):
                ...
                nl.store(X_out_re[b, o, 1, (tile_size_pixels*p):(tile_size_pixels*(p+1))], value=res_sb)
